function fillAndColorIndividualIndices(indexName, sheet) {
  console.log(`Working on ${indexName}`)
  const data = calculateIndividualIndiceColoringBookData(indexName, dataVals)
  const { calculatedColors, calculatedData } = data

  colorIndividualIndiceCellBackgrounds(indexName, calculatedColors, INDICES_MERGE_LENGTH)

  fillIndividualIndiceData(indexName, cbSheet, calculatedData)

  setFormattingForIndividualIndiceCells(indexName, cbSheet, calculatedColors, calculatedData)
}
...
function fillIndividualIndiceData(indexName, sheet, calculatedData) {
  ...
  const range = sheet.getRange(DATA_START_ROW, startingColNum, onlyCalculatedData.length, numCols)
  range.setValues(onlyCalculatedData)
}

function setFormattingForIndividualIndiceCells(indexName, sheet, calculatedColors, calculatedData) {
  const onlyCalculatedData = removeDateData(calculatedData)
...
function colorIndividualIndiceCellBackgrounds(indexName, calculatedColors, numCols) {
  const startingColNum = cbHeaders.indexOf(indexName) + 1
  const onlyColors = removeDateData(calculatedColors)
  colorRange(cbSheet, DATA_START_ROW, startingColNum, onlyColors.length, numCols, onlyColors)
}
...
function getTextStyleForIndividualIndiceCells(stats) {
  ...
  if (+stats.per >= 0) {
    return [TEXT_STYLE_MED_DARK, TEXT_STYLE_PERC_UP, TEXT_STYLE_MED_DARK, TEXT_STYLE_MED_DARK, TEXT_STYLE_MED_DARK]
  } else if (+stats.per <= 0) {
    return [TEXT_STYLE_MED_DARK, TEXT_STYLE_PERC_DN, TEXT_STYLE_MED_DARK, TEXT_STYLE_MED_DARK, TEXT_STYLE_MED_DARK]
  }

  return [TEXT_STYLE_MED_DARK, TEXT_STYLE_MED_DARK, TEXT_STYLE_MED_DARK, TEXT_STYLE_MED_DARK, TEXT_STYLE_MED_DARK]
}
